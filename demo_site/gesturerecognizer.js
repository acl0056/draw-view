...
var JSTapGestureRecognizer = JSTouchRecognizer.extend({
  ...
  touchstart: function(event) {
  	// ACL Edit: With jQuery on non-mobile, only take left click.
	if(jQuery && event.which != 1 && !MobileSafari)
	  return;
	// End ACL Edit
  ...
  touchend: function(event) {
  	  // ACL Edit: With jQuery on non-mobile, only take left click.	
	  if(jQuery && event && event.which != 1 && !MobileSafari)
	    return;
	  // End ACL Edit
...
var JSPanGestureRecognizer = JSGestureRecognizer.extend({
  ...
  touchstart: function(event) {
	...
	// End ACL Edit
    if (event.target == this.target) {
      // ACL Edit: It can be useful to have this data on touchstart.
      this.translationOrigin = this.getEventPoint(event);
      this.dx = 0;
      this.dy = 0;
	  // End ACL Edit
  ...
  touchmove: function(event) {
    ...
    if (allTouches.length >= this.minimumNumberOfTouches &&
      ...
      if (this.beganRecognizer == false) {
        ...
        var p = this.getEventPoint(event);
        // ACL Edit: It can be very useful to have this data on touchmove.
        this.dx = p.x - this.translationOrigin.x,
        this.dy = p.y - this.translationOrigin.y;
        // End ACL Edit
