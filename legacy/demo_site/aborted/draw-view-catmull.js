/*
	...
	TODO:
		...
		Improve temp drawing
		Bug with clear and undo/redo
		Loading document, missing segment start points
*/
function DrawView(options) {
	...
	*/
	...
	this.initializeCatmullCurve = function(v1, v2, v3, v4) {
		var t = 0.5;
		
		p0 = v2.x;
		p1 = t*(v3.x - v1.x);
		p2 = 3*(v3.x - v2.x) - t*(v4.x - v2.x) - 2*t*(v3.x - v1.x);
		p3 = -2*(v3.x - v2.x) + t*(v4.x - v2.x) + t*(v3.x - v1.x);
		
		q0 = v2.y;
		q1 = t*(v3.y - v1.y);
		q2 = 3*(v3.y - v2.y) - t*(v4.y - v2.y) - 2*t*(v3.y - v1.y);
		q3 = -2*(v3.y - v2.y) + t*(v4.y - v2.y) + t*(v3.y - v1.y);
	}
	this.catmullCurveInterp = function(t) {
		var t2 = t*t, t3 = t2*t;
		return {x:p0 + p1*t + p2*t2 + p3*t3, y:q0 + q1*t + q2*t2 + q3*t3};
	}
	...
	this.drawCurve = function(vStart, vEnd) {
		...
		var curve = this.catmullCurveInterp;//this.curveInterp;
...
/* 			this.t2 = tangentForPoints(p0, p1, p2, p3, p4); */
	...
	this.panGestureCallback = function(sender){
		if (sender.state == JSGestureRecognizerStateEnded || sender.state == JSGestureRecognizerStateChanged
			...
			var vq = this.vq;
			var initCatmull = this.initializeCatmullCurve;
			var catmullCurve = this.catmullCurveInterp;
			vq.push(point);
			...
			if ( sender.state == JSGestureRecognizerStatePossible) {
				...
				if (count == 3) {
					...
					// we are checking index 4 and we need 5 and 6 to do so.  That's a count of 7 before we can draw the curve.
					//
					...
					var corner = 1.618, hasCorner = false;
					...
					if (!hasCorner) {
						...
						initCatmull(points[2],points[3],points[5],points[6]);
						var distancePrev = distance(points[3], points[4]),
							t = distancePrev/(distancePrev+distance(points[4],points[5])),
							error = distance(points[4], catmullCurve(t) ),
						...
						if (removeKnot) {
							...
							for (var i=0; i<len; i++) {
								...
								distancePrev = distance(points[3], removed);
								t = distancePrev/(distancePrev+distance(removed, points[5]));
								error = distance(removed, catmullCurve(t));
								if (error >= maxError) {
									removeKnot = false;
									break;
								}
...
/*
						...
						this.initializeCurve(vq.t1, vq.t2);
*/
						initCatmull(points[0],points[1],points[2],points[3]);
				...
				if (points.length > 7) { // change to five
